...
class ProductDetailSizeColorCell: UITableViewCell {
    ...
    //MARK:- Variables
    /*var arraySize : [String] = ["S","M","L","XL","XXL","XXXL"]
    var arrayColors : [String] = [Colors.orange.value,Colors.red.value,Colors.gray.value,Colors.black.value,Colors.green.value]
    var arrayImage : [String] = ["sample"]*/
...
//        {
            ...
            /*for i in 0..<arrayAttributes.count
            {
                
                if(i == 0)
                {
                    //color
                    let dictColor = arrayAttributes[i]
                    lblColor.text = dictColor["name"].stringValue
                    
                    isShowColor = true
                    arrayColors = dictColor["options"].arrayObject as? [String] ?? []
